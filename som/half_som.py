...
class SOM():
    def __init__(self, df, N):
        ...
        with open(os.path.join(new_dir_path, 'half-bmu.csv'), 'w') as f:
            writer = csv.writer(f)
        ...
        for df3 in range(2):
            ...
            for i in range(int(self.n_teacher/2)):
                ...
                with open(os.path.join(new_dir_path, 'half-bmu.csv'), 'a') as f: 
                    writer = csv.writer(f)
                    writer.writerow([i,bmu,y[i]])
                
                df2 = pd.read_csv(os.path.join(new_dir_path, 'half-bmu.csv'))
        ...
        plt.ylim(0-1, N+1)
        #plt.legend()
        plt.savefig(os.path.join(new_dir_path, f'half-Rand'+str(length)+'Seq'+str(count)+
